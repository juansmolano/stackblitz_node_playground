...
class MifareTools {
    ...
    static extractDataFromReadResponse(readResponse, { bNr, readCounter, writeCounter, ti, ext, key, iv, keyMac, keyEnc }) {
        ...
        const blocks = new Array(ext).fill(0).reduce((blocks, index) => {
            blocks[index + bNr] = blocks.slice(index * 16, (index * 16 )+ 16);
        }, {});


        return { data, processVars: { readCounter: readCounter + 1 } };
    }
