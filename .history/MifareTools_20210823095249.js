...
class MifareTools {
    ...
    static extractParametersFromFirstAuthResult(firstAuthResult) {

    }

    static generateReadCmdApdu(bNr, ext, { readCounter, ti, keyMac }) {
        //Read encrypted, MAC on response, MAC on command
        const cmd = 0x31;
        const [bNB1, bNB2] = this.decomposeNumberInTwoBytes(bNr);
        const [rCountB1, rCountB2] = this.decomposeNumberInTwoBytes(readCounter);
        const readCmdCmac = this.calcCmac(keyMac, [rCountB2, rCountB1, ...ti, bNB2, bNB1, ext]);
