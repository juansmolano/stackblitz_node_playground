...
class MifareTools {
    ...
    static generateReadCmdApdu(ext, { readCounter, ti, keyMac, bNr }) {
        ...
        return [cmd, bNB2, bNB1, ext, ...readCmdCmac];
    }

    static extractDataFromReadResponse(readResponse, { bNr, readCounter, writeCounter, ti, ext, key, iv,keyMac }) {
        console.log('-------',readResponse);
        const encriptedData = readResponse.slice(1, -8);
        const respCmac = readResponse.slice(-8);
        const [bNB1, bNB2] = this.decomposeNumberInTwoBytes(bNr);
        const [rCountB1, rCountB2] = this.decomposeNumberInTwoBytes(readCounter);
        const [wCountB1, wCountB2] = this.decomposeNumberInTwoBytes(writeCounter);
        const calCmac = this.calcCmac(keyMac, [readResponse[0], rCountB2, rCountB1, ...ti, bNB2, bNB1, ext, ...encriptedData]);
        ...
        return this.decrypt(encriptedDataToDecrypt, key, ivDec);
    }
