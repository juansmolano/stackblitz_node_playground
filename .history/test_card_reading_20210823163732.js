...
const serverLogicByStep = {
    ...
    0: (paymentMedium, specification, currentStep, responseApdus) => {
        ...
        const { min: afcMinBlock, max: afcMaxBlock } = getMinMaxBlock(specification.APP_AFC.data);
        const { min: pubMinBlock, max: pubMaxBlock } = getMinMaxBlock(specification.APP_PUB.data);
        const block = Math.min(afcMinBlock, pubMinBlock);
        const ext = Math.max(afcMaxBlock, pubMaxBlock) - block;
        const { apdu, processVars } = MifareTools.generateFirstAuthPhaseIApdu(block, true);
        nextStep.apdus.push(MifareTools.bytesToHex(apdu));
        paymentMedium.emissionSteps.processVars = { step: 0, processVars: {block,ext} };
