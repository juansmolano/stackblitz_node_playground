...
const { expand, tap, map, takeUntil, takeWhile, pluck } = require("rxjs/operators");
...
const processServerRequest$ = (currentStep, apdus) => {
    ...
    return from(httpRequest(SMARTCARD_APDUS_END_POINT, { method: 'POST', headers: { 'Content-Type': 'application/json' } }, readerCommand)).pipe(
        ...
        tap(console.log),
        tap(({ nextStep, apdus }) => console.log({ nextStep, apdus })),
        map(({ nextStep, apdus }) => stepServerFn(paymentMedium,currentStep,apdus))
    );
};
