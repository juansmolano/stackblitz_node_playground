...
const serverLogicByStep = {
    'init': (paymentMedium, step, responseApdus) => {
        console.log('=====', { step, responseApdus });
        return { nextStep: 0, apdus: ['FFCA000000'] };
    },
    0: (paymentMedium, step, responseApdus) => {
        const next = { nextStep: null, apdus: [] };

        /* Validate Response:  UUID */
        const responseApdu = (responseApdus || [])[0];
        if (!responseApdu || responseApdu.isValid) {
            next.error = { msg: `No fue posible extraer UUID.  request=${next.apdu} response=${next.response}` };
            return next;
        }
        const uuid = responseApdu.slice(0, -4).toUpperCase();
        if (paymentMedium.mediumId !== uuid) {
            next.error = { msg: `UUID de la tarjeta no coincide con el UUID original.  original=${paymentMedium.mediumId} uuid=${uuid}` };
            return next;
        }
