...
const toHex = (arr) => arr.map(v => v.toString(16));
...
const { keyEnc, keyMac } = MifareTools.extractKeyEV0SessionVars(rndA, rndB, key);
const equals = (a, b) => a.length === b.length && a.every((v, i) => v === b[i]);
console.log({
    keyEnc: toHex(keyEnc), keyEncPreCalculated: toHex(keyEncPreCalculated),
    keyMac: toHex(keyMac), keyMacPreCalculated: toHex(keyMacPreCalculated),
...
const cmac = MifareTools.calcCmac(keyMac, payload);
console.log({
    encryptedData: toHex(encryptedData),
    payload: toHex(payload), cmac: toHex(cmac), cmacPreCalculated: toHex(cmacPreCalculated),
