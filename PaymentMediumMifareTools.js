...
class PaymentMediumMifareTools {
  ...
  paymentMediumToCardDataMap(paymentMedium, endUser, profile) {
    ...
    // ZERO default values
    const historicalFields = Object.keys(
      this.mappingVersion.mapping.data
    ).filter(k => k.startsWith('HIST'));
  ...
  cardDataMapToBinary(cardDataMap) {
    const binaryData = Object.entries(this.mappingVersion.mapping.data).reduce(
  ...
  binaryToCardDataMap(binaryData) {
    ...
    return Object.entries(this.mappingVersion.mapping.data).reduce(
  ...
  isValueBlock(blockData) {
    ...
    return (
      ...
      [[v0, i0], [v1, i1], [v2, i2], [v3, i3]].every(([v, i]) =>
        (~parseInt(v, 16) & 0xff).toString(16).toUpperCase() ===
        i.startsWith('0')
          ? i.substring(1)
          : i
      ) &&
